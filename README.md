...
Seam, for example, evaluates page parameters first, then restrict expressions (analogous to ADVs) and then page actions (see [Seam pages.xml](http://shrubbery.homeip.net/c/display/W/Seam+pages.xml)). My Seam application (which I'm porting to CDI) works without any problems with these authorization requirements.
